// Shared chat prompt templates and builders.
//
// These are used in two places:
// - Client (useChatContext): suggested questions + a local preview of the prompt.
// - Server (api/_lib/chatPrompt.js): the AUTHORITATIVE system prompt. The
//   /api/chat endpoint never accepts a client-supplied system prompt; it
//   rebuilds it here from validated IDs so the endpoint cannot be repurposed
//   as a general Claude proxy (MCR-390).
//
// Pure functions only. No React, no browser APIs, importable from Vercel
// serverless functions via plain relative imports.
...
export function buildLearnContext({
  subsection,
  completedIds,
  llmResult,
  messageCount,
...
export function buildPracticeContext({ practiceContext, title, messageCount, isSocratic }) {
...
// Canonical practice-chat context for a (possibly scenario-merged or
// generated) company object. Used by the client for suggested questions and
// by the server to assemble the practice prompt from a validated company.
export function buildPracticeChatContext(co) {
  return {
    companyId: co._generated ? null : co.id,
    scenarioId: co._scenarioId || null,
    generatedCompany: co._generated ? co : null,
    companyName: co.name,
