...
// Lazy: SoftGate only renders pre-learn, and ChatDrawer pulls react-markdown
// plus the learn chat stack; neither should sit in the practice-route chunk.
const SoftGate = lazy(() => import("../components/onboarding/SoftGate"));
const ChatDrawer = lazy(() => import("../components/learn/ChatDrawer"));
...
export default function PracticeScreen({ session }) {
  ...
  return (
    <div>
      ...
      {co._generated && <GeneratedWarningsBanner key={co.id} warnings={co._warnings} />}
      ...
      <div className="flex items-center justify-between gap-4 mb-2">
        <div>
          <div className="flex items-center gap-3">
            ...
            <span className="text-[10px] uppercase tracking-widest px-3 py-1 rounded-full bg-secondary-container text-on-secondary-container">
              {co.industry}
            </span>
            {co._scenarioName && (
          ...
          {co._scenarioDescription && (
            <p className="text-sm text-on-secondary-container mt-1 bg-secondary-container/30 rounded-lg px-3 py-1.5">
              {co._scenarioDescription}
            </p>
          )}
        </div>
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            onClick={finishCompany}
            className="px-4 py-2 text-[11px] uppercase tracking-widest font-semibold bg-surface-container-low text-on-surface-variant rounded-lg hover:bg-surface-container-high transition-colors"
          >
            Finish
          </button>
        </div>
        ...
        <div className="flex-1 min-w-0">
          <div className={chatOpen ? "space-y-6" : "grid grid-cols-5 gap-6"}>
            ...
            <div className={chatOpen ? "" : "col-span-3"}>
              ...
              <div className="grid grid-cols-2 gap-4 mt-4">
                <div className="bg-surface-container-lowest ghost-border rounded-xl p-4">
                  <h3 className="text-[10px] uppercase tracking-widest text-error font-semibold mb-3">
                    Red Flags
                  </h3>
                  <ul className="space-y-2">
                    {co.redFlags.map((f, i) => (
                      <li key={i} className="text-xs text-on-surface-variant flex gap-2">
                        <span className="material-symbols-outlined text-[14px] text-error shrink-0 mt-0.5">
                          warning
                        </span>
                        <span>{f}</span>
                      </li>
                ...
                <div className="bg-surface-container-lowest ghost-border rounded-xl p-4">
                  <h3 className="text-[10px] uppercase tracking-widest text-on-tertiary-container font-semibold mb-3">
                    Positive Signals
                  </h3>
                  <ul className="space-y-2">
                    {co.greenFlags.map((f, i) => (
                      <li key={i} className="text-xs text-on-surface-variant flex gap-2">
                        <span className="material-symbols-outlined text-[14px] text-on-tertiary-container shrink-0 mt-0.5">
                          check_circle
                        </span>
                        <span>{f}</span>
                      </li>
            ...
            <div className={chatOpen ? "" : "col-span-2"}>
              <h2 className="text-[10px] uppercase tracking-widest text-on-surface-variant font-semibold mb-3">
                Analysis Questions
              </h2>
              <div className="space-y-4">
                {shuffledQuestions.map((q, i) => (
                  <QuestionCard
                    ...
                    index={i}
                    onScore={handleScore}
                    companyContext={buildCompanyContext(co)}
                  />
                ))}
              </div>
            </div>
