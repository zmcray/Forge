...
export default function WeakSpotCard({ weakSpots, quantitativeAccuracy }) {
  ...
  return (
    <div className="bg-surface-container-lowest border border-amber-200 dark:border-amber-900/60 rounded-xl p-4">
      <h3 className="text-sm font-semibold text-amber-900 dark:text-amber-200 mb-2">Focus Areas</h3>
      {weakSpots && weakSpots.map(w => {
        ...
        return (
          <div key={w.type} className="flex items-center gap-2 text-sm mb-1.5">
            ...
            <span className="text-amber-700 dark:text-amber-400 font-mono text-xs">avg {w.avg.toFixed(1)}/5</span>
          </div>
        );
      })}
